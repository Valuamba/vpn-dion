...
class PaginatorCD(CallbackData, prefix='paginator'):
    page: int


class InlineKeyboardPaginator(InlineMarkupConstructor):
    ...
    def __init__(self, page_count, current_page=1):
    ...
    def _to_button_array(self, keyboard_dict):
        ...
        for key in keys:
            if self.current_page == key:
                keyboard.append({
                    'text': str(keyboard_dict[key]), 'callback_data': 'none'
                })
            else:
                keyboard.append({
                    'text': str(keyboard_dict[key]), 'callback_data': PaginatorCD(page=key).pack()
                })
        return keyboard# _buttons_to_dict(keyboard)
    ...
    def get_markup(self, after_schema: []):
        """InlineKeyboardMarkup"""
        keyboards = []

        keyboards += self._keyboard_before
        keyboards += self.keyboard
        keyboards += self._keyboard_after
        ...
        schema = [len(self.keyboard)] + after_schema
        return self.markup(keyboards, schema)
    ...
    def add_after(self, inline_buttons: []):
        ...
        Returns:
            None
        """
        self._keyboard_after += inline_buttons
        # self._keyboard_after.append(_buttons_to_dict(inline_buttons))


def _buttons_to_dict(buttons):
    return [ { 'text': button['text'], 'callback_data': button['callback_data'] } for button in buttons ]