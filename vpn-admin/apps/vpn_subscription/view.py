...
from apps.vpn_subscription.serializers import VpnSubscriptionSerializer, CalculatePaymnetDataSerializer, \
...
def calculate_payment_details(request):
    ...
    discount_percentage = round(100 - ((discounted_price * 100) / discounted_price))
    ...
    # for device in payment_details['devices']:
    #     country = VpnCountry.objects.get(pkid=device['country_id'])
